...
const getBooks = async (req, res, next) => {
  try {
    ...
    const allLibros = await Book.find().populate("authors"); // ESTO SE DEBE PONER SI SE RELACIONAN LAS COLECCIONES
...
const getBookByAuthor = async (req, res, next) => {
  try {
    ...
    return res.status(400).json({message: 'Error al obtener el libro por autor', message: error.message})
  }
}
...
const getBookByCategory = async (req, res, next) => {
  try {
    ...
    const genreFormatted = genre.charAt(0).toUpperCase() + genre.slice(1).toLowerCase();
...
const newBook = async (req, res, next) => {
  try {
    ...
    return res.status(400).json({message: 'Error al crear un nuevo libro en el proyecto6', error: error.message})
  }
}
  ...
  try {
    ...
    const libroUpdate = await Book.findByIdAndUpdate(
      id, 
      {
        ...
        $addToSet: {        // Usamos $addToSet para evitar duplicados en el array
    ...
    if (!libroUpdate) {
      return res.status(404).json({message: 'Libro no encontrado en proyecto6', error: error.message});
    }
    ...
    console.error(error);
    return res.status(400).json({message: 'Error al editar el libro en proyecto6', error: error.message});
  }
};
...
const deleteBook = async (req, res, next) => {
  try {
    ...
    return res.status(400).json({message: 'Error al eliminar el libro', error: error.message});
  }
}
